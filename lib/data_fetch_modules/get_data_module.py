...
def get_historical_klines(client, symbol, interval, start_str, end_str=None):
    ...
    # setup the max limit
    limit = constants.MAX_KLINES_TO_FETCH

    # convert interval to useful value in seconds
    timeframe = util.interval_to_milliseconds(interval)

    # convert our date strings to milliseconds
    start_ts = util.date_to_milliseconds(start_str)

    # if an end time was passed convert it
    end_ts = None
    if end_str:
        end_ts = util.date_to_milliseconds(end_str)
